...
export class ParseIntPipe implements PipeTransform<string, number | undefined> {
  ...
  transform(value: string): number | undefined {
    ...
    if (value === undefined || value === null || value === '') {
      ...
      throw new BadRequestException(errorMessage ?? `${paramName} is required`);
    }
...
> {
  ...
  transform(value: string): boolean | undefined {
    ...
    throw new BadRequestException(
      errorMessage ??
        `${paramName} must be a boolean value (true/false, 1/0, yes/no)`,
    );
  }
}
...
@Injectable()
export class ParseArrayPipe<T = string> implements PipeTransform<
  string | string[],
  T[]
> {
